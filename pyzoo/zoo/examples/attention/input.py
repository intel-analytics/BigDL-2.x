...
class InputFeatures(object):
  ...
  def __init__(self,
    ...
    self.input_mask = np.array(input_mask).reshape((1, 1, len(input_mask)))
    self.segment_ids = np.array(segment_ids)
    self.pos_ids = np.array(pos_ids)
    self.label_id = np.array(label_id)
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  pos_ids = range(0, max_seq_length)
