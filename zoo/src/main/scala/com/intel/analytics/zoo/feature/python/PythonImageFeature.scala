...
class PythonImageFeature[T: ClassTag](implicit ev: TensorNumeric[T]) extends PythonBigDL[T] {
  ...
  def readImageSet(path: String, sc: JavaSparkContext, minPartitions: Int,
                   resizeH: Int, resizeW: Int, flag: Int): ImageSet = {
    if (sc == null) {
      ImageSet.read(path, null, minPartitions, resizeH, resizeW, flag)
    } else {
      ImageSet.read(path, sc.sc, minPartitions, resizeH, resizeW, flag)
    }
  }
