...
class Conv2D extends KerasBaseSpec {
  ...
  "Conv2D NCHW" should "be the same as Keras" in {
    val kerasCode =
      """
        |inputTensor = Input(shape=[3, 24, 24])
        |input = np.random.random([2, 3, 24, 24])
        |outputTensor = Convolution2D(64, 2, 5, activation="relu",
        |                              dim_ordering="th")(inputTensor)
        |model = Model(input=inputTensor, output=outputTensor)
      """.stripMargin
    val seq = Sequential[Float]()
    val layer = Conv2D[Float](64, 2, 5, activation = "relu",
  ...
  "Conv2D NHWC" should "be the same as Keras" in {
    val kerasCode =
      """
      ...
      """.stripMargin
    val seq = Sequential[Float]()
    val layer = Conv2D[Float](32, 4, 6, dimOrdering = "tf",
  ...
  "Conv2D without bias" should "be the same as Keras" in {
    val kerasCode =
      """
        |inputTensor = Input(shape=[3, 24, 24])
        |input = np.random.random([2, 3, 24, 24])
        |outputTensor = Conv2D(64, 2, 5, bias=False, subsample=(2, 3),
        |                              init="normal", dim_ordering="th")(input_tensor)
        |model = Model(input=input_tensor, output=output_tensor)
      """.stripMargin
    val seq = Sequential[Float]()
    val layer = Conv2D[Float](64, 2, 5, subsample = (2, 3),
      init = "normal", useBias = false, inputShape = Shape(3, 24, 24))
    seq.add(layer)
    checkOutputAndGrad(seq.asInstanceOf[AbstractModule[Tensor[Float], Tensor[Float], Float]],
...
class Conv2DSerialTest extends ModuleSerializationTest {
  override def test(): Unit = {
    val layer = Conv2D[Float](64, 2, 5, inputShape =
      Shape(3, 24, 24))
