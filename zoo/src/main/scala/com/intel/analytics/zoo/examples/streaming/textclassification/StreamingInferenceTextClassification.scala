...
object StreamingTextInference {
  ...
  def main(args: Array[String]) {
    ...
    parser.parse(args, TextClassificationParams()).map { param =>
      ...
      // Load bigDL model
      model.doLoad(param.model.get)
      model.modelQueue.take().asInstanceOf[FloatModel]
      ...
      lines.foreachRDD { lineRdd =>
        if (!lineRdd.partitions.isEmpty) {
          ...
          transformed.toDistributed().rdd.foreach { text =>
            val predictSet = model.doPredict(text.getSample.feature())
            predictSet.toTable.foreach(println)
          }
        }
      }
      ssc.start()
