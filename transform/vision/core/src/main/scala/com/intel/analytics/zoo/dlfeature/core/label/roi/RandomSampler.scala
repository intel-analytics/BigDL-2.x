...
import com.intel.analytics.zoo.dlfeature.core.image.{Crop, Feature}
import com.intel.analytics.zoo.dlfeature.core.util.NormalizedBox
...
object RandomSampler {
  ...
  def sampleBbox(feature: Feature): NormalizedBox = {
    val roiLabel = feature(Feature.label).asInstanceOf[RoiLabel]
  ...
  def apply(): Crop = {
    val crop = Crop(roiGenerator = Some(sampleBbox))
    crop.setLabelTransfomer(RoiCrop())
  }
}
