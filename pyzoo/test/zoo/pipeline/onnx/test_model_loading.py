...
class TestModelLoading(OnnxTestCase):
    ...
    def test_averagepool2d_default(self):
        node = onnx.helper.make_node(
            'AveragePool',
            inputs=['x'],
            outputs=['y'],
        ...
        padded = x
        y = pool_op_common.pool(padded, x_shape, kernel_shape, strides, out_shape, (0, 0), 'AVG')
        output = OnnxLoader.run_node(node, [x])
        np.testing.assert_almost_equal(output["y"], y, decimal=5)

    def test_averagepool2d_pads(self):
        node = onnx.helper.make_node(
            'AveragePool',
            inputs=['x'],
            outputs=['y'],
        ...
        y = pool_op_common.pool(padded, x_shape, kernel_shape, strides, out_shape, pad_shape, 'AVG')
        output = OnnxLoader.run_node(node, [x])
        np.testing.assert_almost_equal(output["y"], y, decimal=5)

    def test_averagepool2d_sameupper(self):
        node = onnx.helper.make_node(
            'AveragePool',
            inputs=['x'],
            outputs=['y'],
        ...
        pad_shape = pool_op_common.get_pad_shape('SAME_UPPER', x_shape[2:], kernel_shape, strides, out_shape)
    ...
    def test_averagepool2d_samelower(self):
        import pytest
        with pytest.raises(Exception) as e_info:
            ...
            kernel_shape = (2, 2)
            strides = (1, 1)
            out_shape = pool_op_common.get_output_shape('SAME_LOWER', x_shape[2:], kernel_shape, strides)
            pad_shape = pool_op_common.get_pad_shape('SAME_LOWER', x_shape[2:], kernel_shape, strides, out_shape)
    ...
    def test_averagepool2d_strides(self):
        node = onnx.helper.make_node(
            'AveragePool',
            inputs=['x'],
            outputs=['y'],
