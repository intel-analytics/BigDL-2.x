...
class Rotate(rotationAngles: Array[Double])
  ...
  override def transformTensor(tensor: Tensor[Float]): Tensor[Float] = {
    // def apply[@specialized(Float, Double) T: ClassTag](src: Tensor[T])
    ...
    for (i <- 1 to tensor.size(1)) {
      ...
      for (k <- 1 to tensor.size(2)) {
        ...
        for (j <- 1 to tensor.size(3)) {
          ...
          //         val a = (tensor.valueAt(ii_0, kk_0, jj_0)).toDouble
          //         val b = (tensor.valueAt(ii_1, kk_0, jj_0)).toDouble
          //         val c = (tensor.valueAt(ii_0, kk_0, jj_1)).toDouble
          //         val d = (tensor.valueAt(ii_1, kk_0, jj_1)).toDouble
          //         val e = (tensor.valueAt(ii_0, kk_1, jj_0)).toDouble
          //         val f = (tensor.valueAt(ii_1, kk_1, jj_0)).toDouble
          //         val g = (tensor.valueAt(ii_0, kk_1, jj_1)).toDouble
          //         val h = (tensor.valueAt(ii_1, kk_1, jj_1)).toDouble

//          val a = (1 - wk) * (1 - wj) * (1 - wi) * ((tensor.valueAt(ii_0, kk_0, jj_0)).toDouble)
//          val b = (1 - wk) * (1 - wj) * wi * ((tensor.valueAt(ii_1, kk_0, jj_0)).toDouble)
//          val c = (1 - wk) * wj * (1 - wi) * ((tensor.valueAt(ii_0, kk_0, jj_1)).toDouble)
//          val d = (1 - wk) * wj * wi * ((tensor.valueAt(ii_1, kk_0, jj_1)).toDouble)
//          val e = wk * (1 - wj) * (1 - wi) * ((tensor.valueAt(ii_0, kk_1, jj_0)).toDouble)
//          val f = wk * (1 - wj) * wi * ((tensor.valueAt(ii_1, kk_1, jj_0)).toDouble)
//          val g = wk * wj * (1 - wi) * ((tensor.valueAt(ii_0, kk_1, jj_1)).toDouble)
//          val h = wk * wj * wi * ((tensor.valueAt(ii_1, kk_1, jj_1)).toDouble)
          if (value == -1.0) {
  ...
  //    // (implicit ev: TensorNumeric[T]): Tensor[T] = {
  //    val dst = Tensor[Float](tensor.size()).fill(0f)
  //    val src_depth = tensor.size(1)
  //    val src_height = tensor.size(2)
  //    val src_width = tensor.size(3)
  //    val xc = (src_width + 1)/2.0
  //    val yc = (src_height + 1)/2.0
  //    val zc = (src_depth + 1)/2.0
  //    var id, jd, kd: Double = 0
  //    var ii_0, ii_1, jj_0, jj_1, kk_0, kk_1: Int = 0
  //    for(k <- 1 to src_depth) {
  //      kd = k
  //      for(j <- 1 to src_height) {
  //        jd = j
  //        for(i <- 1 to src_width) {
  //          var value = -1.0
  //          var ri, rj, rk, wi, wj, wk: Double = 0
  //          id = i
  //          val coord = Tensor[Double](Array[Double](kd-zc, jd-yc, id-xc), Array[Int](3, 1))
  //          val rCoord = rotationTensor * coord
  //
  //          ri = rCoord.valueAt(3, 1)
  //          rj = rCoord.valueAt(2, 1)
  //          rk = rCoord.valueAt(1, 1)
  //
  //          ii_0 = math.floor(ri + xc).toInt
  //          jj_0 = math.floor(rj + yc).toInt
  //          kk_0 = math.floor(rk + zc).toInt
  ...
  //          wk = rk + zc-kk_0
  //
  //          if(ii_1 == src_width + 1 && wi < 0.5) ii_1 = ii_0
  //          else if (ii_1 >= src_width + 1) value = 0.0
  //          if(jj_1 == src_height + 1 && wj < 0.5) jj_1 = jj_0
  //          else if (jj_1 >= src_height + 1) value = 0.0
  //          if(kk_1 == src_depth + 1 && wk < 0.5) kk_1 = kk_0
  //          else if (kk_1 >= src_depth + 1) value = 0.0
  //
  //          if(ii_0 == 0 && wi > 0.5) ii_0 = ii_1
  //          else if (ii_0 < 1)value = 0.0
  //          if(jj_0 == 0 && wj > 0.5) jj_0 = jj_1
  ...
  //
  //          if (value == -1.0) {
  //            value = (1 - wk) * (1 - wj) * (1 - wi) * (tensor.valueAt(kk_0, jj_0, ii_0)).toDouble
  //            + (1 - wk) * (1 - wj) * wi * (tensor.valueAt(kk_0, jj_0, ii_1)).toDouble
  //            + (1 - wk) * wj * (1 - wi) * (tensor.valueAt(kk_0, jj_1, ii_0)).toDouble
  //            + (1 - wk) * wj * wi * (tensor.valueAt(kk_0, jj_1, ii_1)).toDouble
  //            + wk * (1 - wj) * (1 - wi) * (tensor.valueAt(kk_1, jj_0, ii_0)).toDouble
  //            + wk * (1 - wj) * wi * (tensor.valueAt(kk_1, jj_0, ii_1)).toDouble
  //            + wk * wj * (1 - wi) * (tensor.valueAt(kk_1, jj_1, ii_0)).toDouble
  //            + wk * wj * wi * (tensor.valueAt(kk_1, jj_1, ii_1)).toDouble
  //          }
  //          dst.setValue(k, j, i, value.toFloat)
  //        }
  //      }
  //    }
  //    dst
