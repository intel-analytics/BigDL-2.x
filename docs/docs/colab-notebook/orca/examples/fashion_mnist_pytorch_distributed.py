...
# <a href="https://colab.research.google.com/github/sgwhat/analytics-zoo/blob/colab/docs/docs/colab-notebook/orca/examples/fashion_mnist_pytorch_distributed.ipynb" target="_parent"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab"/></a>
...
orca_estimator = Estimator.from_torch(model=model_creator, optimizer=optimizer_creator, loss=criterion, metrics=[Accuracy()], backend="torch_distributed")
...
for stat in stats:
  writer.add_scalar("Training Loss", stat['train_loss'], stat['epoch'])
...
for res in val_stats:
    print(res, ":", val_stats[res])


# The accuracy of this model has reached 87%.
...
#.run_line_magic('tensorboard', "--logdir '/content/runs/fashion_mnist_experiment_1'")
