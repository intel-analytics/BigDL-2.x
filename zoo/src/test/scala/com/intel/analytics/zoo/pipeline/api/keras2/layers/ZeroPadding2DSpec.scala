...
class ZeroPadding2DSpec extends KerasBaseSpec {
  ...
  "ZeroPadding2D NCHW" should "be the same as Keras" taggedAs(Keras2Test) in {
    val kerasCode =
      """
        |input_tensor = Input(shape=[2, 8, 8])
        |input = np.random.random([3, 2, 8, 8])
        |output_tensor = ZeroPadding2D(padding=(2, 1), data_format="channels_first")(input_tensor)
  ...
  "ZeroPadding2D NCHW asymmetric" should "be the same as Keras" taggedAs(Keras2Test) in {
    val kerasCode =
      """
        |input_tensor = Input(shape=[2, 4, 5])
        |input = np.random.random([3, 2, 4, 5])
        |output_tensor = ZeroPadding2D(padding=(2, 1, 3, 2),
  ...
  "ZeroPadding2D NHWC" should "be the same as Keras" taggedAs(Keras2Test) in {
    val kerasCode =
      """
      ...
      """.stripMargin
    val seq = Sequential[Float]()
    val layer = ZeroPadding2D[Float](dataFormat = "channels_last", inputShape = Shape(6, 8, 1))
    seq.add(layer)
  ...
  "ZeroPadding2D NHWC asymmetric" should "be the same as Keras" taggedAs(Keras2Test) in {
    val kerasCode =
      """
        |input_tensor = Input(shape=[5, 5, 2])
        |input = np.random.random([3, 5, 5, 2])
        |output_tensor = ZeroPadding2D(padding=(1, 2, 3, 4),
        |data_format="channels_first)(input_tensor)
        |model = Model(input=input_tensor, output=output_tensor)
      """.stripMargin
    val seq = Sequential[Float]()
    val layer = new ZeroPadding2D[Float](padding = Array(1, 2, 3, 4), dataFormat = DataFormat.NHWC,
