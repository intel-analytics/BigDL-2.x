...
object Utils {
  ...
  private[chatbot] def readSentence(directory: String)
  : Array[Array[String]] = {
...
object SequencePreprocess {
  ...
  private[chatbot] def fileToWordIdx(fileName: String, dictionary: Dictionary)
  : Iterator[Float] = {
    val words = readWords(fileName)
    words.map(x => dictionary.getIndex(x).toFloat + 1.0f)
  }

  private[chatbot] def readWords(fileName: String): Iterator[String] = {
  ...
  private[chatbot] def load(fileName: String): Array[String] = {
    import scala.io.Source
