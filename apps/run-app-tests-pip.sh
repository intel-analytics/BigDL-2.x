...
#timer
start=$(date "+%s")
${ANALYTICS_ZOO_HOME}/apps/ipynb2py.sh ${ANALYTICS_ZOO_HOME}/../pyzoo/zoo/zouwu/use-case/network_traffic/network_traffic_multivarite_multistep_tcnforecaster
...
sed -i '/get_ipython()/d; /plot./d; /plt./d' ${ANALYTICS_ZOO_HOME}/../pyzoo/zoo/zouwu/use-case/network_traffic/network_traffic_multivarite_multistep_tcnforecaster.py
sed -i "s/epochs=20/epochs=2/g" ${ANALYTICS_ZOO_HOME}/../pyzoo/zoo/zouwu/use-case/network_traffic/network_traffic_multivarite_multistep_tcnforecaster.py
cd ${ANALYTICS_ZOO_HOME}/../pyzoo/zoo/zouwu/use-case/network_traffic/

python ${ANALYTICS_ZOO_HOME}/../pyzoo/zoo/zouwu/use-case/network_traffic/network_traffic_multivarite_multistep_tcnforecaster.py
cd -
...
then
    clear_up
    echo "zouwu network-traffic-multivarite-multistep-tcnforecaster failed"
    exit $exit_status
fi
now=$(date "+%s")
time23=$((now-start))
