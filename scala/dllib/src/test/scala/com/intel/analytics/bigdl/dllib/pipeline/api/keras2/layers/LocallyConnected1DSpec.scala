...
class LocallyConnected1DSpec extends KerasBaseSpec {
  ...
  "LocallyConnected1D" should "be the same as Keras" taggedAs(Keras2Test) in {
    val kerasCode =
      """
  ...
  "LocallyConnected1D without bias" should "be the same as Keras" taggedAs(Keras2Test) in {
    val kerasCode =
      """
      ...
      """.stripMargin
    val seq = Sequential[Float]()
    val layer = LocallyConnected1D[Float]( filters = 64, kernelSize = 4, strides = 2,
      useBias = false, inputShape = Shape(32, 32))
    seq.add(layer)
    checkOutputAndGrad(seq.asInstanceOf[AbstractModule[Tensor[Float], Tensor[Float], Float]],
      kerasCode, weightConverter)
  }
