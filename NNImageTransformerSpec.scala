...
class NNImageTransformerSpec extends FlatSpec with Matchers with BeforeAndAfter {
  ...
  "NNTransformer" should "has correct result with pascal images" in {
  ...
  "NNTransformer" should "has correct result without MatToTensor" in {
    val imageDF = NNImageReader.readImages(pascalResource.getFile, sc)
    assert(imageDF.count() == 1)
    val transformer = Resize(256, 256) -> CenterCrop(224, 224) ->
  ...
  "NNTransformer" should "ensure imf2Row and Row2Imf reversible" in {
    val imageDF = NNImageReader.readImages(pascalResource.getFile, sc)
    assert(imageDF.count() == 1)
    val transformer = Resize(256, 256) -> CenterCrop(224, 224) ->
      ChannelNormalize(123, 117, 104, 1, 1, 1) -> MatToTensor()
    val transformedDF = new NNImageTransformer(transformer)
      ...
      .transform(imageDF)
    val r = transformedDF.select("features").rdd.first().getAs[Row](0)
    val convertedR = NNImageSchema.imf2Row(NNImageSchema.row2IMF(r))

    assert(r.getSeq[Float](5).toArray.deep == convertedR.getAs[Array[Float]](5).deep)
  }

  "NNTransformer" should "transform gray scale image" in {
    val resource = getClass().getClassLoader().getResource("gray/gray.bmp")
    val df = NNImageReader.readImages(resource.getFile, sc)
    val nnTransformer = new NNImageTransformer(Resize(28, 28) -> MatToTensor[Float]())
      .setInputCol("image")
      .setOutputCol("features")
    val r = nnTransformer.transform(df).select("features").rdd.first().getAs[Row](0)
  ...
  "NNTransformer" should "report error with same input and output columns" in {
    val resource = getClass().getClassLoader().getResource("gray/gray.bmp")
    val df = NNImageReader.readImages(resource.getFile, sc)
    val nnTransformer = new NNImageTransformer(Resize(28, 28) -> MatToTensor[Float]())
      .setInputCol("image")
    ...
    intercept[IllegalArgumentException] {
      val transformed = nnTransformer.transform(df)
    }
  }
