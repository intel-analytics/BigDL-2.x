...
def get_data():
    _sample_mul = 100
    _sample_sign = np.random.randint(100, 200)
    def get_df():
        pass
    
    def get_ts_df():
        df = pd.DataFrame({"datetime": pd.date_range('1/1/2019', periods=_sample_sign),
                                "value": np.random.randn(_sample_sign),
                                "id": np.array(['00']*_sample_sign),
                                "extra feature": np.random.randn(_sample_sign)})        
        return df
    
    def get_multip_df():
        df = pd.DataFrame({"value": np.random.randn(_sample_mul),
                            "id": np.array(['00']*50 + ['01']*50),
                            "extra feature": np.random.randn(_sample_mul)})
        df["datetime"] = pd.date_range('1/1/2019', periods=_sample_mul)
    ...
    def get_no_pd_dt():
        samp_num = np.random.randint()
        df = pd.DataFrame({"datetime":np.random.randn(samp_num),
                        "value":np.random.randn(samp_num),
                        "id":np.array(['00']*samp_num),
                        "extra feature":np.random.randn(samp_num)})
        return df

    def get_no_aligned():
        df_val = pd.DataFrame({"id":np.array(['00']*100),
        ...
        data_min = pd.DataFrame({"datetime": pd.date_range(start='1/2/2019 00:00:00',periods=30,freq='M')})
        data_hou = pd.DataFrame({"datetime": pd.date_range(start='1/3/2019 00:00:00',periods=50,freq="H")})
    ...
    get_df.get_no_aligned = get_no_aligned
    get_df.get_no_pd_dt = get_no_pd_dt
    get_df.get_ugly_ts_df = get_ugly_ts_df
    get_df.get_ts_df = get_ts_df
    get_df.get_multip_df = get_multip_df
    get_df._sample_mul = _sample_mul
    get_df._sample_sign = _sample_sign
    return get_df


class TSDataCreator:

    def __init__(self,dt_col="datetime",target_col="value",extra_feature_col="extra feature",id_col="id"):
        self.dt_col = dt_col
        self.target_col = target_col
        self.extra_feature_col = extra_feature_col
        self.id_col = id_col
        self.id_sensitive = False
        self._horizon = random.randint(1, 10)    
        self._lookback = random.randint(1, 20)

    def get_tsdataset(self,data):
        self.tsdata = TSDataset.from_pandas(data,dt_col=self.dt_col,
                                            target_col=self.target_col,
                                            extra_feature_col=self.extra_feature_col,
                                            id_col=self.id_col)
        return self
    
    def check_shape(self):
        assert self.tsdata._id_list not in ['00','01','0']
        assert self.tsdata.feature_col == ["extra feature"]
        assert self.tsdata.target_col == ["value"]
        assert self.tsdata.dt_col == "datetime"
        assert self.tsdata._is_pd_datetime
        self.tsdata._check_basic_invariants()
    
    def check_dt_feature(self):
        self.tsdata.gen_dt_feature()
        assert set(self.tsdata.to_pandas().columns) == {'IS_AWAKE(datetime)',
                                                   ...
                                                   'datetime',
                                                   'id'}
        assert set(self.tsdata.feature_col) == {'IS_AWAKE(datetime)',
                                           'IS_BUSY_HOURS(datetime)',
                                           'HOUR(datetime)',
                                           'DAY(datetime)',
                                           'IS_WEEKEND(datetime)',
...
class TestTSDataset(ZooTestCase):
    ...
    def test_tsdataset_initialization(self):
        # legal input
        self.tsdata.extra_feature_col=["extra feature"]
        self.tsdata.get_tsdataset(self.data.get_ts_df())
        self.tsdata.check_shape()

        self.tsdata.target_col=["value"]
        self.tsdata.get_tsdataset(self.data.get_ts_df())
        self.tsdata.check_shape()

        self.tsdata.id_col=None
        self.tsdata.get_tsdataset(self.data.get_ts_df())
        self.tsdata.check_shape()

        # illegal input
        with pytest.raises(AssertionError):
            self.tsdata.target_col,self.tsdata.id_col=["value"],0
            self.tsdata.get_tsdataset(self.data.get_ts_df())
        with pytest.raises(AssertionError):
            self.tsdata.dt_col, self.tsdata.target_col=0,["value"]
            self.tsdata.get_tsdataset(self.data.get_ts_df())
        with pytest.raises(AssertionError):
            self.tsdata.target_col=0
            self.tsdata.get_tsdataset(self.data.get_ts_df())
        with pytest.raises(AssertionError):
            self.tsdata.target_col=["value"]
            self.tsdata.get_tsdataset(0)
        with pytest.raises(AssertionError):
            self.tsdata.target_col=["value1"]
            self.tsdata.get_tsdataset(self.data.get_ts_df())

    def test_tsdataset_initialization_multiple(self):
        # legal input
        self.tsdata.extra_feature_col=["extra feature"]
        self.tsdata.get_tsdataset(self.data.get_multip_df())
        self.tsdata.check_shape()
    ...
    def test_tsdataset_roll_single_id(self):
        ...
        with pytest.raises(RuntimeError):
            self.tsdata.tsdata.to_numpy()

        # roll train
        self.tsdata.tsdata.roll(lookback=lookback, horizon=horizon)
        x, y = self.tsdata.tsdata.to_numpy()
        assert x.shape == (self.data._sample_sign-lookback-horizon+1, lookback, 2)
        assert y.shape == (self.data._sample_sign-lookback-horizon+1, horizon, 1)

        self.tsdata.tsdata.roll(lookback=lookback, horizon=horizon, id_sensitive=True)
        x, y = self.tsdata.tsdata.to_numpy()
        assert x.shape == (self.data._sample_sign-lookback-horizon+1, lookback, 2)
        assert y.shape == (self.data._sample_sign-lookback-horizon+1, horizon, 1)
        ...
        x, y = self.tsdata.tsdata.to_numpy()
        assert x.shape == (self.data._sample_sign-lookback-horizon+1, lookback, 1)
        assert y.shape == (self.data._sample_sign-lookback-horizon+1, horizon, 1)
        ...
        assert x.shape == (self.data._sample_sign-lookback-horizon+1, lookback, 2)
        assert y is None
        self.tsdata.tsdata._check_basic_invariants()
    ...
    def test_tsdataset_scale_unscale(self):
        df, df_test = self.data.get_ts_df(),self.data.get_ts_df()
