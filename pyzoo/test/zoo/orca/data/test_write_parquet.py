...
from zoo.orca.data.image import write_mnist, write_voc, write_imagenet
...
def test_write_imagenet(orca_context_fixture, use_api=False):
    sc = orca_context_fixture
    temp_dir = tempfile.mkdtemp()
    try:
        data_dir = os.path.join(resource_path, "imagenet/ILSVRC2018/CLS-LOC/train")
        map_class_path = os.path.join(resource_path, "imagenet/devkit/data/map_clsloc.txt")
        if use_api:
            write_parquet("imagenet", "file://" + temp_dir, data_dir=data_dir,
                          map_class_path=map_class_path)
        else:
            write_imagenet(data_dir, map_class_path, "file://" + temp_dir)
...
def test_write_parquet_api(orca_context_fixture):
    ...
    test_write_imagenet(orca_context_fixture, True)
