...
def load_dien_data(data_dir):
    ...
    n_uid = usertbl.df.select("id").agg({"id": "max"}).collect()[0][0] + 1
    n_mid = itemtbl.df.select("id").agg({"id": "max"}).collect()[0][0] + 1
    n_cat = cattbl.df.select("id").agg({"id": "max"}).collect()[0][0] + 1
    train_data.show()
...
if __name__ == '__main__':
    ...
    estimator.fit(train_data, epochs=args.epochs, batch_size=args.batch_size,
                  feature_cols=feature_cols, label_cols=['label'], validation_data=test_data)
