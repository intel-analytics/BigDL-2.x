...
/**
 * [[RecurrentExtension]] module is a container of rnn cells
 ...
 *
 */
class RecurrentExtension[T : ClassTag](
  ...
  protected val batchDim = RecurrentExtension.batchDim
  protected val timeDim = RecurrentExtension.timeDim
...
//  }
  ...
  override def updateOutput(input: Tensor[T]): Tensor[T] = {
    ...
    while (i <= times) {
      currentInput(inputDim) = RecurrentExtension.selectCopy(input2Cell, i, stepInput2CellBuf)
    ...
    RecurrentExtension.copy(cells.map(x => x.output.toTable[Tensor[T]](inputDim)),
      output)
    output
  }
  ...
  override def accGradParameters(input: Tensor[T], gradOutput: Tensor[T]): Unit = {
    ...
    var i = times
    while (i >= 1) {
      currentGradOutput(inputDim) = RecurrentExtension.selectCopy(gradOutput, i, stepGradBuffer)
      _input(hidDim) = if (i > 1) cells(i - 2).output.toTable(hidDim)
      else if (initHiddenState == null) hidden else initHiddenState
      _input(inputDim) = RecurrentExtension.selectCopy(input2Cell, i, stepInput2CellBuf)
      ...
      if (maskZero && i > minLength) {
        ...
        for (b <- 1 to curMask.size(1)) {
          if (curMask(Array(b, 1)) == ev.zero) {
            val originState = gradOutputBuff[Table](RecurrentExtension.hidDim)
  ...
  override def updateGradInput(input: Tensor[T], gradOutput: Tensor[T]): Tensor[T] = {
    ...
    RecurrentExtension.copy(cells.map(x => x.gradInput.toTable[Tensor[T]](inputDim)), gradInput2Cell)
    if (preTopology != null) {
  ...
  override def canEqual(other: Any): Boolean = other.isInstanceOf[RecurrentExtension[T]]

  override def equals(other: Any): Boolean = other match {
    case that: RecurrentExtension[T] =>
      super.equals(that) &&
        (that canEqual this) &&
        cells == that.cells
    case _ => false
...
object RecurrentExtension extends ContainerSerializable {
  ...
  def apply[@specialized(Float, Double) T: ClassTag](
    ...
    (implicit ev: TensorNumeric[T]) : RecurrentExtension[T] = {
    new RecurrentExtension[T](batchNormParams, maskZero = maskZero)
  }
  ...
  override def doLoadModule[T: ClassTag](context : DeserializeContext)
    ...
    val recurrent = if (flag) {
      RecurrentExtension[T](BatchNormParams[T]())
    } else {
      RecurrentExtension[T]()
    }
  ...
  override def doSerializeModule[T: ClassTag](context: SerializeContext[T],
    ...
    val recurrent = context.moduleData.module.asInstanceOf[RecurrentExtension[T]]
