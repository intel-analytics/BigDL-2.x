...
import com.intel.analytics.bigdl.nn.{ClassNLLCriterionExtension, RecurrentExtension, _}
  ...
  def main(args: Array[String]): Unit = {
    trainParser.parse(args, new TrainParams()).map(param => {
      ...
      val padLabel = Tensor[Float](T(padId))
        ...
        val encoder =
          Array(
            RecurrentExtension(maskZero = true).add(LSTM(param.embedDim, param.embedDim)),
            RecurrentExtension(maskZero = true).add(LSTM(param.embedDim, param.embedDim)),
            RecurrentExtension(maskZero = true).add(LSTM(param.embedDim, param.embedDim))
          )
      ...
      val optimizer = Optimizer(
        ...
        criterion = TimeDistributedMaskCriterion(
          new ClassNLLCriterionExtension(paddingValue = padId, sizeAverage = false),
          paddingValue = padId
        ),
