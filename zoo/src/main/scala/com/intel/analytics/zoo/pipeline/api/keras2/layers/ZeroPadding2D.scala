 ...
 *                Default is (1, 1, 1, 1).
 * @param dataFormat Format of the input data. Either DataFormat.NCHW ('channels_first') or
...
class ZeroPadding2D[T: ClassTag](
  ...
  extends klayers1.ZeroPadding2D[T](padding, dimOrdering = dataFormat, inputShape) with Net {}
...
object ZeroPadding2D {
  def apply[@specialized(Float, Double) T: ClassTag](
    ...
    new ZeroPadding2D[T](paddingArray, KerasUtils.toBigDLFormat(dataFormat), inputShape)
  }
}
