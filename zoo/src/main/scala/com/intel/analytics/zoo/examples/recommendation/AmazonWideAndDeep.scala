  ...
  def run(params: WNDParams): Unit = {
    ...
    */
    val localColumnInfo = ColumnFeatureInfo(
      wideBaseCols = Array("category", "price", "vote"),
      wideBaseDims = Array(2, 3, 3),
      wideCrossCols = Array("category-price"),
      wideCrossDims = Array(100),
      indicatorCols = Array("category"),
      indicatorDims = Array(2),
      embedCols = Array("userId", "itemId", "vote"),
      embedInDims = Array(userCount, itemCount, 3),
      embedOutDims = Array(64, 64, 64),
      continuousCols = Array("price"))
