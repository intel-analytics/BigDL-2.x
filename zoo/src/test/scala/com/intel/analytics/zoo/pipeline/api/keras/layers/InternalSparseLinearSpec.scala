...
import com.intel.analytics.bigdl.nn.{InternalSparseLinear, Linear, Identity => BIdentity,
ParallelTable => BParallelTable, Sequential => BSequential, SparseJoinTable => BSparseJoinTable}
...
class InternalSparseLinearSpec extends FlatSpec with Matchers {
  "Sparse Linear" should "return the same result with Linear" in {
    ...
    val sl = InternalSparseLinear(4, 2)
    val l = Linear(4, 2)
    ...
    out1 should be (out2)
    sl.parameters()._2.last should be (l.parameters()._2.last)

    val expectedG = Tensor.sparse(l.parameters()._2.head)
    assert(sl.sparseParameters()._2.head.equals(expectedG) == true)
  }
  ...
  "Sparse Linear" should "return the same result with Linear 3" in {
    val gradOutput = Tensor(2, 2).rand()
    val input = Tensor(2, 4).rand()
    val sl = InternalSparseLinear(4, 2, backwardStart = 1, backwardLength = 4)
    val l = Linear(4, 2)
    l.weight.copy(sl.weight)
    l.bias.copy(sl.bias)
    val sparseInput = Tensor.sparse(input)
  ...
  "Sparse Linear" should "return the same result with Linear 5" in {
    ...
    val sl = InternalSparseLinear(10, 2, backwardStart = 5, backwardLength = 5)
    val l = Linear(10, 2)
    l.weight.copy(sl.weight)
    l.bias.copy(sl.bias)
    val sparseInput = Tensor.sparse(input)
  ...
  "Sparse Linear" should "return the same result with Linear 6" in {
    ...
    val sl = InternalSparseLinear(5, 2, backwardStart = 1, backwardLength = 5)
    val sparseModel = BSequential().add(BParallelTable().add(BIdentity()).add(BIdentity()))
      .add(BSparseJoinTable(2))
      .add(sl)
    val l = Linear(5, 2)
    l.weight.copy(sl.weight)
    l.bias.copy(sl.bias)
  ...
  "Sparse Linear" should "return the same result with Linear 7" in {
    ...
    val sl = InternalSparseLinear(1023263, 2, backwardStart = 1, backwardLength = 1023263)
    val sj = BSparseJoinTable(2)
    val sparseModel = BSequential().add(BParallelTable().add(BIdentity()).add(BIdentity()))
      .add(sj)
      .add(sl)
    ...
    assert(Tensor.dense(sl.sparseParameters()._2.head)
      .almostEqual(l.parameters()._2.head, 1e-6) == true)
  }
}

class InternalSparseLinearSerialTest extends ModuleSerializationTest {
  override def test(): Unit = {
    val sparseLinear = InternalSparseLinear[Float](4, 2).setName("internalSparseLinear")
