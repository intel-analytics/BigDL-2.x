...
sed "s/end_trigger=MaxEpoch(10)/end_trigger=MaxEpoch(5)/g; s/movielen\s_data = movielen\s.get_id_rating\s\(\"\/tmp\/movielen\s\/\"\)/movielen\s_data = \(movielen\s.get_id_rating\s\(\"\/tmp\/movielen\s\/\"\)\)[0:50000:]/g" ${ANALYTICS_ZOO_HOME}/apps/recommendation-ncf/ncf-explicit-feedback.py >${ANALYTICS_ZOO_HOME}/apps/recommendation-ncf/tmp.py
