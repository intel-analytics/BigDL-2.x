...
_WORKER_NUM=$2  # worker num on each numa node
...
if [ $TOTAL_CORE_NUM -lt $_WORKER_CORE_NUM_LESS_THAN ] && [ -z "${_WORKER_NUM}" ]; then
  # use local mode
  echo "local[*]"
  exit 1
fi
...
for nnode in ${_NUMA_HARDWARE_INFO[@]}; do
  if [[ ${nnode} =~ ^node\ ([0-9]+)\ cpus:\ (.+)$ ]]; then
    ...
    if [[ -z "${_WORKER_NUM}" ]]; then
      ...
      _WORKER_NUM=$((_LENGTH / $((_WORKER_CORE_NUM_LESS_THAN / 2))))
    fi
