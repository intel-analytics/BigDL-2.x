...
class Seq2seq[T: ClassTag](
  ...
  decoder: Decoder[T],
  inputShape: Shape,
  bridges: KerasLayer[Activity, Activity, T] = null)
  ...
  override def buildModel(): AbstractModule[Table, Tensor[T], T] = {
    ...
    val decoderInitStates = SelectTable(2).inputs(encoderOutput)
...
object Seq2seq {
  def apply[@specialized(Float, Double) T: ClassTag](
    ...
    new Seq2seq[T](encoder, decoder, inputShape, bridges).build()
  }
}