...
import com.intel.analytics.bigdl.{serialization, _}
...
/**
  * [[RecurrentDecoderExtension]] module is a container of rnn cells that used to make
  ...
  */
class RecurrentDecoderExtension[T : ClassTag](val seqLength: Int)
  (implicit ev: TensorNumeric[T]) extends RecurrentExtension[T] {
  ...
  RecurrentDecoderExtension.this.type = {
  ...
  override def updateOutput(input: Tensor[T]): Tensor[T] = {
    ...
    RecurrentExtension.copy(cells.map(x => x.output.toTable[Tensor[T]](inputDim)), output)
    output
  }
  ...
  override def accGradParameters(input: Tensor[T], gradOutput: Tensor[T]): Unit = {
    ...
    while (i >= 1) {
      currentGradOutput(inputDim) = if (i == times) {
        RecurrentExtension.selectCopy(gradOutput, i, stepGradBuffer)
      } else {
        val _gradInput = cells(i).gradInput.toTable[Tensor[T]](inputDim)
        RecurrentExtension.selectCopy(gradOutput, i, stepGradBuffer).add(_gradInput)
      }
  ...
  override def updateGradInput(input: Tensor[T], gradOutput: Tensor[T]): Tensor[T] = {
    ...
    RecurrentExtension.copy(cells.map(x => x.gradInput.toTable[Tensor[T]](inputDim)), gradInput)
    gradInput
  }
  ...
  override def canEqual(other: Any): Boolean = other.isInstanceOf[RecurrentDecoderExtension[T]]

  override def equals(other: Any): Boolean = other match {
    case that: RecurrentDecoderExtension[T] =>
      super.equals(that) &&
        (that canEqual this) &&
        cells == that.cells
    case _ => false
...
object RecurrentDecoderExtension extends ContainerSerializable {
  def apply[@specialized(Float, Double) T: ClassTag](outputLength: Int)
                                                    (implicit ev: TensorNumeric[T]): RecurrentDecoderExtension[T] = {
    new RecurrentDecoderExtension[T](outputLength)
  }
  ...
  override def doLoadModule[T: ClassTag](context: DeserializeContext)
    ...
    val recurrentDecoder = RecurrentDecoderExtension[T](seqLen.getInt32Value)
  ...
  override def doSerializeModule[T: ClassTag](context: SerializeContext[T],
    ...
    val recurrentDecoder = context.moduleData.module.asInstanceOf[RecurrentDecoderExtension[T]]
