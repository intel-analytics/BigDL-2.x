...
class DoppelGANgerGenerator(nn.Module):
    ...
    def __init__(self, feed_back, noise,
        ...
        if attribute_dim is None:
            ...
            if len(self.addi_attribute_outputs) > 0:
                self.all_attribute_input_noise_dim = [attribute_input_noise_dim, addi_attribute_input_noise_dim]
                self.all_attribute_outputs = [self.real_attribute_outputs, self.addi_attribute_outputs]
                self.all_attribute_out_dim = [self.real_attribute_out_dim, self.addi_attribute_out_dim]
            else:
        ...
        self.mlp_1 = None # Metadata Generator if mlp_2 is not None, Min/Max Generator if mlp_2 is None
        self.mlp_2 = None # Min/Max Generator if not None
        for part_i in range(len(self.all_attribute_input_noise_dim)):
            ...
            if len(all_discrete_attribute_dim) > 0:
                layers_dim = [self.all_attribute_input_noise_dim[part_i] + all_discrete_attribute_dim[-1]]
            else:
            ...
            for i in range(len(layers_dim) - 1):
                ...
                mlp_list.append(nn.BatchNorm1d(layers_dim[i+1], eps=1e-05, momentum=0.1))  # 0.9 or 0.1?
        ...
        self.rnn_network = nn.LSTM(input_size=self.attribute_out_dim + feature_input_noise_dim,
                                   ...
                                   batch_first=True)  # use batch_first to reduce too much transpose
    ...
    def _post_process_generated_attribute(self, sub_attribute_output, sub_all_attribute_outputs):
        ...
        for i in range(len(sub_all_attribute_outputs)):
            ...
            if output.type_ == OutputType.DISCRETE:
                ...
                sub_output_discrete = F.one_hot(torch.argmax(sub_output, dim=1), num_classes=output.dim) # batch_size, output.dim, 0/1
            ...
            current_idx += output.dim
        assert current_idx == np.sum([t.dim for t in sub_all_attribute_outputs])  # should be commented
        part_attribute = torch.cat(part_attribute, dim=1)
        part_discrete_attribute = torch.cat(part_discrete_attribute, dim=1)
        part_discrete_attribute = part_discrete_attribute.detach() # discrete attribute will be used as other generator's input
    ...
    def forward(self, attribute_input_noise, addi_attribute_input_noise,
        ...
        part_additional_discrete_attribute = None  # post-processed additional_attribute_output(discrete)
        ...
        if self.mlp_1 is not None and self.mlp_2 is not None:
            ...
            # generate real attr
            attribute_output = self.mlp_1(attribute_input_noise) # (batch_size, real_attribute_out_dim)
            ...
            # generate addi attr
            addi_attribute_generator_input = torch.cat([part_real_discrete_attribute, addi_attribute_input_noise], dim=1)
            additional_attribute_output = self.mlp_2(addi_attribute_generator_input) # (batch_size, addi_attribute_out_dim)
            part_additional_attribute, part_additional_discrete_attribute = \
                self._post_process_generated_attribute(additional_attribute_output, all_attribute_outputs[1])

        elif self.mlp_1 is not None and self.mlp_2 is None:
            additional_attribute_output = self.mlp_1(attribute_input_noise) # (batch_size, addi_attribute_out_dim)
            assert len(all_attribute_outputs) == 1
            part_additional_attribute, part_additional_discrete_attribute = \
                self._post_process_generated_attribute(additional_attribute_output, all_attribute_outputs[0])
        ...
        all_attribute = torch.cat(all_attribute, dim=1) # will be used in Auxiliary Discriminator
        all_discrete_attribute = torch.cat(all_discrete_attribute, dim=1) # will be send to RNN, should with out grad
        all_discrete_attribute = all_discrete_attribute.detach() # just to make sure it is detached, maybe not needed
        ...
        for i in range(time):
            ...
            input_all = torch.cat(input_all, dim=1) # (batch_size, attribute_out_dim + feature_noise_dim)
            input_all = torch.unsqueeze(input_all, dim=1) # (batch_size, 1, attribute_out_dim + feature_noise_dim)
            ...
            for j in range(self.sample_len):
                ...
                all_cur_argmax.append(torch.argmax(
                                      new_output_all[(j * len(self.feature_outputs) +
        ...
        feature = feature.view(batch_size, time * self.sample_len, int(self.feature_out_dim / self.sample_len))
