...
import com.intel.analytics.zoo.pipeline.api.keras.layers.EmbeddingGloVe
  ...
  def main(args: Array[String]): Unit = {
    ...
    parser.parse(args, TextClassificationParams()).map { param =>
      ...
      val model = if (param.model.isDefined) {
        ...
        val embedding = EmbeddingGloVe[Float]("glove.6B." + tokenLength.toString + "d",
          gloveDir, wordIndex, false, null)
        TextClassifier(classNum, embedding, sequenceLength,
