...
class FeatureTable(Table):
    ...
    def filter_by_frequency(self, columns, min_freq=2):
        ...
        key = freq_df.groupby(columns).count()
        group = key.filter(key['count'] >= min_freq)
    ...
    def hash_encode(self, columns, bins, method='md5'):
        ...
        :param columns: str or a list of str, the target columns to be encoded. 
               For dense feature, you need to cut them into discrete intervals beforehand.
    ...
    def cross_hash_encode(self, new_col_name, columns, bins, method='md5'):
        ...
        :param new_col_name: str, new column which is generated by columns to be crossed(i.e. 'A_B', 'C_D').
