...
class NNEstimatorSpec extends ZooSpecHelper {
  ...
  "An NNEstimator" should "has correct default params" in {
    val df = NNFileReader.readCSV("/home/ding/proj/AsiaInfo/manyidu/fakeData/", sc)
    val path = "/home/ding/proj/AsiaInfo/manyidu/"
    val yuyinPath = "xgb_yuyin-18-16.model"
    val model2 = NNXGBoostClassifierModel.load(path + yuyinPath, 3)
    ...
    val shouji_model = NNXGBoostClassifierModel.load(path + shoujiPath, 3)
    ...
    val zifei_model = NNXGBoostClassifierModel.load(path + zifeiPath, 3)
    ...
    val manyi_model = NNXGBoostClassifierModel.load(path + zifeiPath, 3)
