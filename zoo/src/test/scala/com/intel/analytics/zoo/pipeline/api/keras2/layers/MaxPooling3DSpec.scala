...
class MaxPooling3DSpec extends KerasBaseSpec {
  ...
  "MaxPooling3D" should "be the same as Keras" taggedAs(Keras2Test) in {
    val kerasCode =
      """
        |input_tensor = Input(shape=[3, 20, 15, 35])
        |input = np.random.random([2, 3, 20, 15, 35])
        |output_tensor = MaxPooling3D((2, 2, 3), data_format="channels_first")(input_tensor)
...
class MaxPooling3DSerialTest extends ModuleSerializationTest {
  override def test(): Unit = {
    ...
    val input = Tensor[Float](2, 3, 20, 15,
      35).apply1(_ => Random.nextFloat())
