...
class AveragePooling2DSpec extends KerasBaseSpec {
  ...
  "AveragePooling2D NCHW" should "be the same as Keras" taggedAs(Keras2Test) in {
    val kerasCode =
      """
        |input_tensor = Input(shape=[3, 24, 24])
        |input = np.random.random([2, 3, 24, 24])
        |output_tensor = AveragePooling2D(data_format="channels_first")(input_tensor)
        |model = Model(input=input_tensor, output=output_tensor)
      """.stripMargin
    val seq = Sequential[Float]()
    val layer = AveragePooling2D[Float](poolSize = (2, 3), strides = (1, 2),
      padding = "same", dataFormat = "channels_first", inputShape = Shape(3, 24, 44))
    seq.add(layer)
  ...
  "AveragePooling2D NHWC" should "be the same as Keras" taggedAs(Keras2Test) in {
    ...
    val seq = Sequential[Float]()
    val layer = AveragePooling2D[Float](poolSize = (2, 3), strides = (1, 2),
      padding = "same", dataFormat = "channels_last", inputShape = Shape(20, 32, 4))
    seq.add(layer)
