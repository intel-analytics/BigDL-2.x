...
class DetectionOutputMRcnn(val confidence: Double = 0.7, val nmsThresh: Float = 0.3f,
  ...
  def refineDetection(_rois: Tensor[Float], probs: Tensor[Float],
    ...
    val refinedRois = BboxUtil2.bboxTransformInv(rois, deltaSpecific, true)
    BboxUtil.scaleBBox(refinedRois, 1024, 1024)
    // Clip boxes to image window
    val boxes = BboxUtil2.clipToWindows(window, refinedRois)
    ...
    // Apply per-class NMS
    val preNmsClassIds = BboxUtil2.selectTensor(classIds, keep, 1)
    val preNmsScores = BboxUtil2.selectTensor(classScores, keep, 1)
    val preNmsRois = BboxUtil2.selectTensor(boxes, keep, 1)
    ...
    while (i <= classNum) {
      if (preNmsClassIds.storage().array().contains(i.toFloat)) {
        ...
        val scores = BboxUtil2.selectTensor(preNmsScores, ixs, 1).squeeze(2)
        val bboxes = BboxUtil2.selectTensor(preNmsRois, ixs, 1)
    ...
    if (keep.length > 0) {
      ...
      val (sortedScore, sortedIds) =
        BboxUtil2.selectTensor(classScores, keep, 1).topk(keep.length, 1, false)
      ...
      val finalRois = BboxUtil2.selectTensor(refinedRois, keep, 1)
      val finalClassIds = BboxUtil2.selectTensor(classIds, keep, 1) // .apply1(_ - 1)
      val finalScores = BboxUtil2.selectTensor(classScores, keep, 1)
      println(finalScores)
