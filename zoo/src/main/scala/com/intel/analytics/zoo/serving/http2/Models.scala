...
case class modelsPredictionInput(uuid: String, models: Models) extends PredictionInput {
  override def getId(): String = this.uuid
  override def toHash(): HashMap[String, String] = {
    val hash = new HashMap[String, String]()
    val bytes = models.toArrow()
...
object modelsPredictionInput {
  def apply(models: Models): modelsPredictionInput =
    modelsPredictionInput(UUID.randomUUID().toString, models)
}
...
case class Models(models: List[mutable.LinkedHashMap[String, Any]]) {
  if (models == null) {
    throw new ServingRuntimeException("no model can be found, " +
      "please check your input or yaml(keyword should be 'models')")
  }
    ...
    models.map(instance => {
      instance.map(i => {
        ...
        if (value.isInstanceOf[SparseTensor[_]]) {
          ...
          val indicesTensor = Models.transferListToTensor(sparseTensor.indices)
          key -> ((shape, data), indicesTensor)
        } else {
          val tensor =
            if (value.isInstanceOf[List[_]]) {
              Models.transferListToTensor(value)
            } else {
              (new mutable.ArrayBuffer[Int](0), value)
            }
          key -> (tensor, Models.transferListToTensor(List()))
        }
      })
    })
  }
  ...
  def makeSchema(
    ...
    assert(models.size > 0, "must have models, and each should have the same schema")
...
object Models {
  def apply(instance: mutable.LinkedHashMap[String, Any]): Models = {
    Models(List(instance))
  }

  def apply(models: mutable.LinkedHashMap[String, Any]*): Models = {
    Models(models.toList)
  }

  def fromArrow(arrowBytes: Array[Byte]): Models = {
    val models = new mutable.ArrayBuffer[mutable.LinkedHashMap[String, Any]]()
