...
class CaffeLoader[T: ClassTag](prototxtPath: String, modelPath: String,
  ...
  private def registerCustomizedConverter(): Unit = {
    if (customizedConverters != null) {
      customizedConverters.foreach(entry => {
        ...
        entry._2.registorContext("netparam", netparam)
      })
    }
  }

  private def loadCaffe(prototxtPath: String, modelPath: String): Unit = {
    if (isCaffeLoaed) return
  ...
  private val s3aPrefix: String = "s3a:"

  private def getFileSystem(fileName: String): org.apache.hadoop.fs.FileSystem = {
    val src = new Path(fileName)
    val fs = src.getFileSystem(getConfiguration(fileName))
    require(fs.exists(src), src + " does not exists")
    fs
  }

  private def getConfiguration(fileName: String): Configuration = {
    if (fileName.startsWith(hdfsPrefix) || fileName.startsWith(s3aPrefix)) {
      new Configuration()
    } else {
  ...
  private def loadBinary(prototxtPath: String, modelPath: String): Caffe.NetParameter = {
    ...
    try {
      modelFs = getFileSystem(prototxtPath)
      prototxtFs = getFileSystem(prototxtPath)
      ...
      val np = mergeNetWithParams(netBuilder.build, weightBuilder.build)
  ...
  private def mergeNetWithParams(net: NetParameter, weights: NetParameter): NetParameter = {
    ...
    net.getLayersList.asScala.foreach(v1Layer => {
      v1Layers.append(v1Layer)
    })
    net.getLayerList.asScala.foreach(v2Layer => {
      v2Layers.append(v2Layer)
    })

    weights.getLayersList.asScala.foreach(v1Layer => {
      layers(v1Layer.getName) = v1Layer
    })
    weights.getLayerList.asScala.foreach(v2Layer => {
      layers(v2Layer.getName) = v2Layer
    })
...
object FrcnnCaffeLoader {
  ...
  def loadCaffe(defPath: String, modelPath: String): Module[Float] = {
    ...
    val postprocessor = if (proposal.scales.length == 3) {
      // vgg
      DetectionOutputFrcnn(0.3f, 21, false, 100, 0.05)
    } else {
      DetectionOutputFrcnn(0.4f, 21, true, 100, 0.05)
    }
