...
package com.intel.analytics.zoo.pipeline.chatbot

import com.intel.analytics.bigdl.nn.abstractnn.{AbstractModule, Activity, TensorModule}
import com.intel.analytics.bigdl.nn.{RecurrentExtension, RecurrentDecoderExtension, Sequential}
...
import com.intel.analytics.zoo.pipeline.chatbot.DecoderInputType.DecoderInputType

import scala.collection.mutable.ArrayBuffer
import scala.reflect.runtime._

class Seq2seq[T: ClassTag](val encoderRecs: Array[RecurrentExtension[T]],
   val decoderRecs: Array[RecurrentExtension[T]],
  ...
  override def updateOutput(input: Activity): Tensor[T] = {
    val feedbackPreviousOutput = decoderRecs.head.isInstanceOf[RecurrentDecoderExtension[T]]
  ...
  override def backward(input: Activity, gradOutput: Tensor[T]): Tensor[T] = {
    ...
    if (decoderRecs.head.isInstanceOf[RecurrentDecoderExtension[T]]) {
...
object Seq2seq extends ModuleSerializable {
  def apply[@specialized(Float, Double) T: ClassTag](encoderCells: Array[RecurrentExtension[T]],
     decoderCells: Array[RecurrentExtension[T]], preEncoder: AbstractModule[Activity, Activity, T] = null,
  ...
  override def doLoadModule[T: ClassTag](context: DeserializeContext)
    ...
    val encoderCells = DataConverter.
      ...
      asInstanceOf[Array[AbstractModule[_, _, T]]].map(_.asInstanceOf[RecurrentExtension[T]])
