...
```
If you run ./init.sh meeting failure, please try to exit the container and then follow the commands of [docs of PPML](https://github.com/ManfeiBai/analytics-zoo/blob/patch-12/docs/readthedocs/source/doc/PPML/Overview/ppml.md#21-prerequisite), and recreated the keys and password directory following those commands. Then modify those PATHs in the ./deploy-local-spark-sgx.sh, and rerun the ./deploy-local-spark-sgx.sh to build one new container. <br>
