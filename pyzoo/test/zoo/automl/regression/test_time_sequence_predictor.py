...
class TestTimeSequencePredictor(ZooTestCase):
    ...
    def test_fit_SmokeRecipe(self):
        ...
        validation_df = pd.DataFrame({"datetime": pd.date_range('1/1/2019', periods=val_sample_num),
    ...
    # def test_fit_GridRandomRecipe(self):
    #     sample_num = np.random.randint(100, 200)
    ...
    #                                 future_seq_len=future_seq_len,
    #                                 extra_features_col=None, )
    #     pipeline = tsp.fit(train_df, recipe=GridRandomRecipe(num_rand_samples=1, look_back=2))
    #     assert isinstance(pipeline, TimeSequencePipeline)
    #     assert isinstance(pipeline.feature_transformers, TimeSequenceFeatureTransformer)
    #     assert isinstance(pipeline.model, BaseModel)
    #     assert pipeline.config is not None
    ...
    def test_fit_BayesRecipe(self):
        ...
        tsp = TimeSequencePredictor(dt_col="datetime",
                                    ...
                                    future_seq_len=future_seq_len,
                                    extra_features_col=None, )
        pipeline = tsp.fit(train_df, recipe=BayesRecipe(num_samples=1, look_back=(3, 5)))
        assert isinstance(pipeline, TimeSequencePipeline)
        assert isinstance(pipeline.feature_transformers, TimeSequenceFeatureTransformer)
        assert isinstance(pipeline.model, BaseModel)
        assert pipeline.config is not None
